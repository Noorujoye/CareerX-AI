package com.noorain.login_system.service;
...
public class AuthenticationService {
     ...
     *
     * Security note:
     * - We do NOT accept role from the client; the server decides the role.
    ...
    /**
     * Authenticates email/password and returns a fresh JWT.
     */
    public AuthenticationResponse authenticate(AuthenticationRequest request) {
        ...
        // 1) Delegate credential checking to Spring Security
        authenticationManager.authenticate(
                new UsernamePasswordAuthenticationToken(email, request.getPassword())
        );

        // 2) Load the user and mint a token
